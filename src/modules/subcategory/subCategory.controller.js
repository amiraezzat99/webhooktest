...
export const createSubCategory = async (req, res, next) => {
  ...
  if (await subCategoryModel.findOne({ name })) {
    return res
      .status(400)
      .json({ message: 'please enter different subCategory name' })
  }
  ...
  if (!subCategory) {
    ...
    return res
      .status(400)
      .json({ message: 'Fail to add subCategory, please try again later' })
  }
  return res
    .status(201)
...
export const updateSubCategory = async (req, res, next) => {
  ...
  if (req.body.name) {
    if (subCategory.name == req.body.name) {
      return next(
        new Error('please enter different categotry name', { cause: 400 }),
      )
    }
